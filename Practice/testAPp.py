...
def setLonLat(str_argLocation):
    ...
    ## The API i am using (Google Maps) might not support all cities, so for that below's code will print N/A
    if "results" in code.keys():
...
def implement_city_info():
    ...
    if bool_file_empty:
        f_write.write('Name, Population, Longitude, Latitude, Temperature °C \n')
    ...
    f_write.write(str_city_name + ", " + str(str_population).replace(",", "") + ", " + str(double_lng) + ", " + str(
        double_lat) + ", " +
                  int_tempreture + "\n")
    f_write.close()
...
def get_city_names():
    ...
    file = requests.get(
        "http://worldpopulationreview.com/countries/" + str_country_name.lower() + "-population/cities/").text
